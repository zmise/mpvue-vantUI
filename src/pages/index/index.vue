<template>
  ...
  <div class="container">
    ...
    <div class="tab-style">
      <div class="tabs">
        <div v-bind:class="{ active: loading, 'tab-box': showBox }" @click="loading=true">
          智能报价
        </div>
        <div v-bind:class="{ active: !loading, 'tab-box': showBox }" @click="loading=false">
          仓位查询
        </div>
      </div>
      <div class="content" v-show="loading">
        <van-popup :show="show" position="bottom" overlay="false">
          <van-picker show-toolbar title=" " :columns="columns" @change="onChange(1,$event)" @cancel="onClose(1)"
            @confirm="onConfirm(1)" />
        </van-popup>
        <van-cell-group>
          <van-field :value="destinationCountry" disabled label="目的国" @click="show=true" icon="arrow" />
          <van-field :value="piece" clearable label="件数" type="number" placeholder="请输入件数" />
          <van-field :value="weight" clearable label="重量" type="number" placeholder="请输入大于100KG的重量" border="false" />
          <van-field :value="volume" clearable label="体积" type="number" placeholder="m³" border="false" class="teshu" />
        </van-cell-group>
        <van-button round type="danger" size="large">搜索</van-button>
      </div>
      <div class="content" v-show="!loading">
        <van-popup :show="show2" position="bottom" overlay="false">
          <van-picker show-toolbar title=" " :columns="columns2" @change="onChange(2,$event)" @cancel="onClose(2)"
            @confirm="onConfirm(2)" />
        </van-popup>
        <van-popup :show="show3" position="bottom" overlay="false">
          <van-picker show-toolbar title=" " :columns="columns3" @change="onChange(3,$event)" @cancel="onClose(3)"
            @confirm="onConfirm(3)" />
        </van-popup>

        <div class="ub-box ub-flex-end " style="height:70px ;width: 100%;">
          <div class="ub-box ub-col ub-flex-1 ub-ver-v ub-ver">
            <p style="font-size: 15px;color: #999">交货城市</p>
            <input :value="deliveryCity" disabled placeholder="城市" @click="show2=true"
              style="height: 40px;width: 100%;text-align:center;font-size: 24px;font-weight: 700;color: #323332" />
          </div>
          <van-icon name="arrow" class="ub-box ub-col ub-flex-1 ub-ver-v ub-ver"
            style="height: 40px;font-size: 24px;font-weight: 700;color: #323332" />
          <div class="ub-box ub-col ub-flex-1 ub-ver-v ub-ver">
            <p style="font-size: 15px;color: #999">目的城市</p>
            <input :value="destinationCity" disabled placeholder="城市" @click="show3=true"
              style="height: 40px;width: 100%;text-align:center;font-size: 24px;font-weight: 700;color: #323332" />
          </div>
        </div>
        <van-popup :show="show4" position="bottom" overlay="false">

          <van-datetime-picker type="date" :value="currentDate" :min-date="minDate" @change="change"
            @cancel="onClose(4)" @confirm="confirm" />

        </van-popup>
        <div class="itmebox ub-box ub-ver ub-between" @click="show4=true">
          <div style="font-size: 24px;font-weight: 700;color: #323332">{{date}}</div>
          <div style="font-size:15px;color:#bbb;margin-right: 80px;">{{week}}</div>
          <van-icon name="arrow" style="font-size:24px;color:#bbb" />
        </div>
        <div class="itmebox ub-box ub-ver ub-between">
          <div style="font-size: 15px;font-weight: 700;">飞行方式</div>
        ...
        <div class="itmebox ub-box ub-ver ub-between" style="border-bottom: .5px solid #efefef;margin-bottom: 10px;">
          <div style="font-size: 15px;font-weight: 700;">机型</div>
          <van-checkbox-group :value="result1" @change="onSelect(2,$event)" class=" ub-box ub-ver">
            <van-checkbox :wx:for="list1" :wx:key="index" :name="item"
              style="font-size:15px;color:#323332;width:100px;font-weight: 700">
              {{ item }}
            </van-checkbox>
          </van-checkbox-group>
